...
Deno.serve(async (req) => {
  ...
  // Scope gate: this function's admin actions require the 'documents' scope.
  // Synthetic webhook tokens bypass; super + owner roles bypass.
  if (!(payload as { synthetic?: boolean }).synthetic && !(await requireScope(createClient(SUPABASE_URL, SERVICE_ROLE), payload as never, 'documents'))) {
    return jsonResponse({ ok: false, error: 'Missing required scope: documents' }, 403);
  }
