...
Deno.serve(async (req) => {
  ...
  // Scope gate: this function's admin actions require the 'policies' scope.
  // Synthetic webhook tokens bypass; super + owner roles bypass.
  if (!(payload as { synthetic?: boolean }).synthetic && !(await requireScope(sb, payload as never, 'policies'))) {
    return jsonResponse({ ok: false, error: 'Missing required scope: policies' }, 403);
  }
